...
cp -r dataset dataset_temp
find dataset_temp -type f -not -name "*.pdf"
find dataset_temp -type f -name requirements.txt -delete
find dataset_temp -type d -empty -delete
tar -czf dataset_pdf.tar.gz dataset_temp
curl -X PUT --data-binary "@dataset_pdf.tar.gz" -H "Content-Type: application/octet-stream" $PRESIGNED_URL_FOR_BUCKET_FOR_UPLOAD"dataset_pdf.tar.gz"
